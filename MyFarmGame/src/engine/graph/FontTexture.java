...
public class FontTexture {
    ...
    private final String CHAR_SET_NAME;

    private final Map<Character, CharInfo> CHAR_MAP;
    ...
    private void buildTexture() throws Exception {
        ...
        String allChars = getAllAvailableChars(CHAR_SET_NAME);
        this.width = 0;
        this.height = fontMetrics.getHeight();
        for (char c : allChars.toCharArray()) {
            // Get the size for each character and updateSize global image size
            CharInfo charInfo = new CharInfo(width, fontMetrics.charWidth(c));
            CHAR_MAP.put(c, charInfo);
        ...
        int startX = 0;
        for (char c : allChars.toCharArray()) {
            CharInfo charInfo = CHAR_MAP.get(c);
